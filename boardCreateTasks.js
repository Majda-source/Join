...
function displayToDos(search) {
    ...
    for (let i = 0; i < todos.length; i++) {
        ...
        if (!search || title.includes(search)) {
            // wenn es search nicht gibt dann fÃ¼hre aus, und wenn title etwas von der suche beinhaltet dann fÃ¼hre ebenfalls aus, wenn nicht dann zeigt er auch nichts an 
 ...
 */
 function displayImportanceStatus(element, importanceId, white) {
    ...
    if (element['prio'] == 'Urgent') {
        importanceId.src = `./assets/img/arrows-up.svg`;
    }
    if (element['prio'] == 'Medium') {
        importanceId.src = `./assets/img/equal-sign.svg`;
    }
    if (element['prio'] == 'Low') {
        importanceId.src = `./assets/img/arrows-down.svg`;
    }
}
